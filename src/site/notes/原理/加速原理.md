---
{"dg-publish":true,"permalink":"/原理/加速原理/","dgPassFrontmatter":true,"created":"2025-03-18T16:01:43.254+08:00","updated":"2025-03-24T12:14:23.619+08:00"}
---
...
- 在 `/etc/dnsmasq.d/` 下会存在以下文件：
	...
	-  `defaultserver.conf` ：白名单默认DNS，该DNS Server默认用加速隧道的出口的当地DNS Server（默认填充两个DNS Server），但不知道该文件如何生成
...
-  **非本机报文：** `PREROUTING`（ `mangle` -> `nat` ） -> `路由表` -> `POSTROUTING` （ `mangle` -> `nat` ） -> `下一跳` 
- **本机报文：** `OUTPUT` （ `mangle` -> `nat` ） -> `路由表` -> `POSTROUTING` （ `mangle` -> `nat` ） -> `下一跳`

#### 匹配DNS解析文件顺序

- `domain-X.conf` > `dnsserver.conf` > `reslov.conf`
